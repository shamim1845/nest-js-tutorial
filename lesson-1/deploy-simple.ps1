# Simple NestJS Deployment Script for Minikube
param(
...
function Show-Info {
    ...
    Write-Host "  ❤️  Health Check: http://$MINIKUBE_IP:30080/health" -ForegroundColor Green
    Write-Host ""
    Write-Host "🔧 Commands:" -ForegroundColor White
    Write-Host "  kubectl get pods -n nestjs-app" -ForegroundColor Yellow
    Write-Host "  kubectl logs deployment/nestjs-app-deployment -n nestjs-app" -ForegroundColor Yellow
    Write-Host ""
}
...
switch ($Action.ToLower()) {
    ...
    default {
        Write-Host "Usage: .\deploy-simple.ps1 {deploy|status|test|cleanup}" -ForegroundColor White
    }
}
